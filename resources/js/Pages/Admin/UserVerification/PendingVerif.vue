...
const approveVerification = (id) => {
    selectedVerification.value = usersData.value.find(user => user.id === id);
...
const confirmApprove = async () => {
    try {
        await router.post(route('admin.verification.update-status', selectedVerification.value.id));
        // Remove the approved user from the pending list
        usersData.value = usersData.value.filter(user => user.id !== selectedVerification.value.id);
        approveDialogVisible.value = false;
        toast.add({ severity: 'success', summary: 'Success', detail: 'Verification approved successfully', life: 3000 });
    } catch (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: error.response?.data?.message || 'Failed to approve verification', life: 3000 });
    }
};
...
const confirmReject = async () => {
    try {
        ...
        toast.add({ severity: 'error', summary: 'Error', detail: error.response?.data?.message || 'Failed to reject verification', life: 3000 });
    }
};
...
<template>
    <section v-if="!usersData.length" class="p-4 bg-white shadow sm:p-6 sm:rounded-md">
    ...
    <section v-else>
        <DataTable v-model:filters="filters" :value="usersData" removableSort tableStyle="min-width: 100%"
