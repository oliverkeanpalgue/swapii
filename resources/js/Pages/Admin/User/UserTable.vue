...
const acceptDialog = (id) => {
    confirm.require({
        group: 'templating',
        message: 'Set this User to Moderator ?',
        header: 'Change role of User to Moderator',
        ...
        accept: () => {
            router.patch(route('admin.moderator.set', id));
            toast.add({ severity: 'success', summary: 'Success', detail: 'Set moderator', life: 3000 });
            confirm.close();
        },
        reject: () => {
...
<template>
    ...
    <section v-else>
        <DataTable v-model:filters="filters" :value="props.users" removableSort tableStyle="min-width: 100%"
            ...
            <Column field="name" header="Name" class="text-sm" sortable>
                <template #body="{ data }">
                    <div class="flex items-center gap-3">
                        <div v-if="data.profilePicture" class="h-8 w-8 rounded-full overflow-hidden">
                            <img :src="data.profilePicture" :alt="data.name" class="h-full w-full object-cover">
                        </div>
            ...
            <Column field="email" header="Email" class="text-sm" sortable>
                <template #body="{ data }">
                    <div class="text-gray-600">{{ data.email }}</div>
                </template>
            </Column>

            <Column field="created_at" header="Date Joined" class="text-sm" sortable>
                <template #body="{ data }">
